...
const ItemTable = ({ items }: { items: any }) => {
    ...
    const handleCheckboxChange = (itemId: string) => {
        if (selectedItems.includes(itemId)) {
            ...
            // Otherwise, add it to selectedItems
            ...
            <thead>
                <tr className="text-gray-700">
                    <th className="border border-gray-300 px-4 py-2">
                        <input
                            type="checkbox"
                            checked={selectAll} // Reflect selectAll state
                            onChange={handleSelectAll} // Handle selectAll toggle
                        />
                    </th>
                ...
                {items.map((item: any) => (
                    <tr key={item.itemId} className="hover:bg-gray-50">
                        <td className="border border-gray-300 px-4 py-2">
                            <input
                                type="checkbox"
                                checked={selectedItems.includes(item.itemId)} // Reflect if this item is selected
                                onChange={() => handleCheckboxChange(item.itemId)} // Handle individual checkbox toggle
                            />
                        </td>
